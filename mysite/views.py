...
def logMysiteInfo(accessInfo):
    with open(file=r'D:\seldom\rd\Django_school\django1006\static\log\accessLog.log', mode='a',
...
#         else:
#             return render(request,'error_index.html',{'info':'客官您没有权限访问此站点!!'})
#     return render(request,'error_index.html',{'info':'客官您没有权限访问此站点!!'})
...
def userInfoList(request):
    ...
    if countUIL > 0:
        ...
        # 这里的话是第一次从handle_form_index路由过来的时候需要直接返回数据库已有的数据进行展示
            ...
            return redirect(to='/my_error/',request=request)
    return redirect(to='/my_path/',request=request)
...
def userList(request):
    ...
    if username != '' and password != '' and username != None and password != None and username != 'None' and password != 'None':
        users = models.UserInfo.objects.filter(username=username, password=password)
        if users.count() > 0:
            request.session['username'] = username
            request.session['password'] = password
...
# 根据用户名和密码删除用户数据
def userInfoList_delete(request):
    countUIL = session_judge(request)
    if countUIL > 0:
        # 去掉空格
        username_userInfoList_delete = str(request.POST.get('username_userInfoList_delete')).strip()
        password_userInfoList_delete = str(request.POST.get('password_userInfoList_delete')).strip()
        # 这里的话是必须需要四重判断才可以!!!!  != '' 且 != None
        if username_userInfoList_delete != '' and password_userInfoList_delete != '' and username_userInfoList_delete != None and password_userInfoList_delete != None and username_userInfoList_delete != 'None' and password_userInfoList_delete != 'None':
            # index那边来的用户名和我们userInfoList添加的用不用的key名字即可区分....
            usersD = models.UserInfo.objects.filter(username=username_userInfoList_delete,
                                                    password=password_userInfoList_delete)
            if usersD.count() > 0:
                models.UserInfo.objects.filter(username=username_userInfoList_delete,
                                               password=password_userInfoList_delete).delete()
                user_list = list(models.UserInfo.objects.all())
                # 这里的话是第一次从handle_form_index路由过来的时候需要直接返回数据库已有的数据进行展示
                return render(request, 'userInfoList.html', {'data': user_list})
            else:
...
def my_error(request):
    info = '客官您没有权限的说..'
    return render(request,'error_index.html',{'info':info})